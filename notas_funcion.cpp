...
int main() {
    ...
    if (notaIngresadaError) cout << "Alguna de las notas de los ejercicios no es valida";
        ...
        for (int i = 0; i < alumnos; i++) {
            ...
            if (personas[i].notaFinal > notaMasAlta) {

                notaMasAlta = personas[i].notaFinal;
                strcpy(alumnoNotaMasAlta, personas[i].apellido);
            }
        }

        cout << "la nota mas alta de todas es un " << notaMasAlta << ", conseguida por " << alumnoNotaMasAlta;
    }
